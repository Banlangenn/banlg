    ...
    run(cwd, args) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                // 保存文件信息  等待撤回
                yield this.fileSavePromise(path.join(__dirname, this.jsonPath), JSON.stringify({
...
<style lang='scss' >
            ...
            if (isChildren) {
                ...
                traverse(ast, {
                    ObjectExpression(path) {
                        ...
                        properties.forEach(element => {
                            if (element.value && element.value.name === self.argv.parentName) {
                                path.pushContainer('properties', t.objectProperty(t.identifier('children'), t.arrayExpression([self.generateEl(true, true)])));
                                path.skip();
                            }
                        });
                    }
    ...
    generateEl(isChildren = true, isFirst = false) {
        ...
        this.argv.metaParam && propertyArray.push(t.objectProperty(t.identifier('meta'), t.stringLiteral(this.argv.metaParam)), t.objectProperty(t.identifier('name'), t.stringLiteral(this.argv.componentName)));
